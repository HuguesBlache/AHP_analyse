...
def weight_list_geomtrique(data):
    jugment_scale={'Equally':1,'A':2,'Strongly':3,'Extremely':4}
    alpha={'No expertise':1,'Beginner Level':1,'Intermediate Level':1,
    'Advanced Level':1,'Expert Level':1}
    list_of_value={}
    weight=[]
    expertise=[]
    ...
    if len(data['columns_name'])==12:   
        for h in range(0,len(data['list_index'])): 
            ...
            for colums_index in range(4,10):
                filtre_columns=data['data'].iloc[h, colums_index] #### valeurs des questios
                list_of_filtre=filtre_columns
                value_comparer=re.findall(r'\[([^\]]+)\]',data['data'].iloc[:,colums_index].name)[0].lower() ### Quelles est le sujet declarer
                if value_comparer in list_of_filtre: #### Si le mot est dans la reponse alors on evalue que c'est un inverse propose car la value comparer est moins imfluence
            ...
            eigenvalues, eigenvectors =np.linalg.eig(np.array(matrix_ahp))
            ...
            #log_weight=[np.log(weight[i])*expertise[i] for i in range(0,len(expertise))]
            weight_ponderer=sum(weight)/len(weight)
  
        return pd.DataFrame(data={"weight":weight_ponderer},index=nom_value)
        t#pd.DataFrame(data={"weight":apply_betP_test(weight)},index=nom_value)#expertise,matrix_ahp,ahp_normalisation_poids_matrice(np.array(matrix_ahp)),nom_value,pandas_ahp,pandas_ahp_norm,CR_value
...
#### Les comparaison de la deuxieme partie
def weight_list_simple_comparaison_ponderation(data):
    expertise=[]
    weight=[]
    ...
    for h in range(0,len(data['data'])): 
        ...
        if 'T-Intersection' in data['name_comparaison_2']:
            ...
            if value_comparer[0] in list_of_filtre: #### Si le mot est dans la reponse alors on evalue que c'est un inverse propose car la value comparer est moins imfluence
                list_of_value.append(jugment_scale[list_of_filtre.split()[0]])
...
def  weight_list_extra_all_ponderation(data):
    jugment_scale={'Equality':1,'A':2,'Strongly':3,'Extremely':4}
    alpha={'No expertise':1,'Beginner Level':1,'Intermediate Level':1,
    'Advanced Level':1,'Expert Level':1}
    ...
    for i in range(0,len(new_data.columns)):
        ...
        log_weight=[np.log(weight[i])*expertise[i] for i in range(0,len(expertise))]
        weight_ponderer=sum(weight)/len(weight)
        pandas_all.append(pd.DataFrame(data={"weight":weight_ponderer},index=nom_value))
    return pandas_all
...
def confidence_by_scenario_ponderation(data):
    ...
    if 'Stop' in data['columns_name'][0]:
        ...
        value_confidence=sum(value_justement)/len(value_justement)
        value_confidence_2=sum(value_justement_layer_2)/len(value_justement_layer_2)
        ...
        for h in range(0,len(data['data'])): 
            ...
            value_justement_layer_3.append(alpha[data['expertise_leve'][h]]*np.log(np.mean(value_jugement_by_person_layer_3)))
            value_justement.append(alpha[data['expertise_leve'][h]]*np.log(np.mean(value_jugement_by_person)))
            expertise.append(alpha[data['expertise_leve'][h]])
        ...
        value_confidence=sum(value_justement)/len(value_justement)
        value_confidence_3=sum(value_justement_layer_3)/len(value_justement_layer_3)
...
def ahp_classique(survey):
    ...
    for i in list_scenario:
        list_scenario[i]['result_comp_1']=weight_list_geomtrique(list_scenario[i])
        pandas_weight_ponderer.append(weight_list_geomtrique(list_scenario[i]))
        list_scenario[i]['result_comp_2']=weight_list_simple_comparaison_ponderation(list_scenario[i])
        pandas_weight_ponderer_2.append(weight_list_simple_comparaison_ponderation(list_scenario[i]))
    ...
    pandas_weight_ponderer_3=weight_list_extra_all_ponderation(data_survey)
    ...
    list_scenario['Partie_2']['result_comp_1']=weight_list_extra_all_ponderation(data_survey)[0]
    list_scenario['Partie_2']['result_comp_2']=weight_list_extra_all_ponderation(data_survey)[1]
    list_scenario['Partie_2']['result_comp_3']=weight_list_extra_all_ponderation(data_survey)[2]
    list_scenario['Partie_2']['result_comp_4']=weight_list_extra_all_ponderation(data_survey)[3]
    ...
    ### Or Stop = Intersection et Cycle Lane = Cycliste
    ...
    df_category = pd.DataFrame(data_category, index=index_category)


    # Votre liste de listes
    ...
    return resultat_word,resultat_final_normalised,resultat_final,df_category